  ...
  loadDashboardData() {
    ...
    this.auth.getDashboard().subscribe({
      next: (res: any) => {
        const s = res?.stats || {}
        this.stats = [
          { title: 'Total Products', value: s.totalProducts ?? 0, icon: 'package', color: '#f59e0b' },
          { title: 'Total Sales', value: s.totalSales ?? '$0', icon: 'badge-dollar-sign', color: '#10b981' },
          { title: 'Active Orders', value: s.activeOrders ?? 0, icon: 'shopping-cart', color: '#3b82f6' },
